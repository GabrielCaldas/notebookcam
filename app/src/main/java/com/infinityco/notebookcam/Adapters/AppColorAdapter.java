...
public class AppColorAdapter extends RecyclerView.Adapter<AppColorAdapter.DataObjectHolder>{

    private Context context;
    private static int selected;

    private static String LOG_TAG = "MyRecyclerViewAdapter";
    private static AppColorAdapter.MyClickListener myClickListener;
    ...
    public void setOnItemClickListener(AppColorAdapter.MyClickListener myClickListener) {
    ...
    public AppColorAdapter(Context context) {

        this.context = context;
        try {
            selected = new Settings_values(context).getAppColor()-1;
        }
        catch (Exception e){
            selected = 0;
        }
    ...
    @Override
    public AppColorAdapter.DataObjectHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.carditem_folder_colors, parent, false);

        AppColorAdapter.DataObjectHolder dataObjectHolder = new AppColorAdapter.DataObjectHolder(view,context);
        return dataObjectHolder;
    }

    @Override
    public void onBindViewHolder(AppColorAdapter.DataObjectHolder holder, final int position) {
